...
const MetricsCards = () => {
  const { metrics, workers } = useContext(OrchestrationContext);

  const cards = [
    {
      title: "Active Workers",
      value: metrics.activeWorkers ?? workers.length ?? 0,
      color: "#00f5ff"
    },
    {
    ...
    {
      title: "Queue Load",
      value: (metrics.pendingQueue ?? 0) + (metrics.processingQueue ?? 0),
      color: "#3b82f6"
    },
    {
      title: "Failures",
    ...
    {
      title: "Jobs/sec",
      value: Number(metrics.throughput ?? 0).toFixed(2),
      color: "#8b5cf6"
    }
  ];
