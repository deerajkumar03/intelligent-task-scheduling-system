...
const COLORS = ["#3b82f6", "#06b6d4", "#22c55e", "#ef4444"];

const QueueOverview = () => {
  const { metrics } = useContext(OrchestrationContext);

  const data = [
    { name: "Queued", value: metrics.pendingQueue ?? 0 },
    { name: "Processing", value: metrics.processingQueue ?? 0 },
    { name: "Completed", value: metrics.completedJobs ?? 0 },
    { name: "Failures", value: metrics.failedJobs ?? 0 }
  ];
