...
const TelemetryPanel = () => {
  const { logs } = useContext(OrchestrationContext);
  ...
  return (
    <div className="panel telemetry-panel">
      <div className="panel-header">
        <h2>Live Orchestration Timeline</h2>
      </div>

      <div className="telemetry-box">
        {logs.length === 0 ? (
          <div className="empty-workers">Waiting for orchestration events...</div>
        ) : (
          logs.map((log, index) => (
            <div
              className={`telemetry-event ${getTypeClass(log.type)}`}
              key={`${index}-${log.time}`}
            >
              ...
              <div className="telemetry-content">
                ...
                {log.type && <span className="telemetry-type">{log.type}</span>}
